...
import { GuideChapter } from './guide.model';
...
export class GuideComponent implements AfterViewInit {
  ...
  get activeChapter(): GuideChapter | undefined {
    return this.chapters.find(
      chapter => chapter.number === this.activeChapterNumber
    );
  }

  /** Handle chapter selection */
  selectChapter(chapterNumber: number): void {
    if (chapterNumber === this.activeChapterNumber) return;

    this.activeChapterNumber = chapterNumber;

    const chapter = this.activeChapter;
    if (chapter?.steps.length) {
      this.currentImage = chapter.steps[0].image;
    }
  ...
  /** Attach observer to visible sections */
  private observeSections(): void {
    if (!this.observer || !this.sections) return;
